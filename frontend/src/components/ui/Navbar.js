					...
					{({ open }) => (
						<>
								...
								<div className="flex items-center justify-between h-20">
									<NavLink className="flex justify-start items-center " to="/">
										<img
											...
											alt="Jackson Play"
										/>
										<p className='ml-2 text-xl font-bold'>Jackson Play </p>
									</NavLink>
											...
											<div className="ml-10 flex  items-baseline space-x-4 ">
												{navigation.map((item) => (
													<NavLink
														key={item.name}
														to={item.to}
														className={({ isActive }) => ' px-2  text-black text-lg font-medium' + (isActive ? ' active underline decoration-solid decoration-3' : '')}
													>
														{item.name}
													</NavLink>
												))}
							...
							<Disclosure.Panel className="md:hidden">
								...
								{currentUser ? <div className="pt-4 pb-3 border-t border-gray-700">
